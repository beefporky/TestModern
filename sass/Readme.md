...
folder are mapped to the application's root namespace, 'TestModern'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "TestModern"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'TestModern' in the same way
as `"TestModern/sass/src"`.
